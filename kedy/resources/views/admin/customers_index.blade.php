...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h2 class="text-3xl font-semibold text-green-800 mb-6">Müşteriler</h2>
                    ...
                    @foreach($customers as $customer)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('admin.users_show', $customer->id) }}"
                                   class="text-green-600 hover:text-green-900 mr-3">Görüntüle</a>
                                <a href="{{ route('admin.users_edit', $customer->id) }}"
                                   class="text-yellow-600 hover:text-yellow-900 mr-3">Düzenle</a>
                                <form action="{{ route('admin.users_destroy', $customer->id) }}" method="POST"
                                      class="inline-block">
                                    @csrf
                                    @method('DELETE')
